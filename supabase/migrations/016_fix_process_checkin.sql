-- Migración para corregir la lógica de process_checkin
-- Fecha: 2025-09-09
-- Propósito: Corregir el flujo de racha completada para que inicie en 1 después de completar

-- ============================================
-- CORREGIR FUNCIÓN process_checkin
-- ============================================
...
BEGIN
  ...
  INTO v_new_current_count, v_previous_expires_at, v_streak_created_at, v_streak_created_at, v_was_just_completed
  FROM public.streaks 
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    -- 🔥 CORRECCIÓN: Manejar el caso "recién completada"
    current_count = CASE
    ...
    max_count = CASE 
      WHEN v_streak_expired OR v_streak_broken OR streaks.is_just_completed THEN 
...
'Función de check-in CORREGIDA: maneja correctamente el reinicio de racha después de completar (inicia en 1)';

-- Verificar que la función se actualizó correctamente
DO $$ 
BEGIN 
  RAISE NOTICE 'Función process_checkin CORREGIDA exitosamente - Flujo de racha completada reparado';
END $$;
