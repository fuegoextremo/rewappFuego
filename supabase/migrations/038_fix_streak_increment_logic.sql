-- ============================================
-- MIGRACION 038: Corregir logica de incremento de racha
-- ============================================
-- Problema: La logica de incremento de racha usa last_check_in de la tabla streaks,
-- pero deberia usar el conteo real de check_ins del dia actual.
-- Esto causa inconsistencias cuando se borra un check-in.
...
BEGIN
  ...
  -- ============================================
  -- 3. VERIFICAR LIMITE DE CHECK-INS DIARIOS
  -- ============================================
  ...
  -- ============================================
  -- 6. OBTENER THRESHOLD MAXIMO CONFIGURADO
  -- ============================================
  ...
  -- Convertir last_check_in a fecha para comparacion
  ...
  -- Verificar si la racha se rompio por inactividad
  IF v_streak_created_at IS NOT NULL AND v_last_checkin_date IS NOT NULL THEN
  ...
  -- ============================================
  -- 8. DETERMINAR SI DEBE INCREMENTAR
  -- ============================================
  -- LOGICA CORREGIDA: Usar conteo real de check-ins de HOY
  -- v_existing_checkins = cuantos habia ANTES de insertar (ya insertamos uno mas)
  -- Si habia 0 antes, este es el primer check-in de hoy -> INCREMENTAR
  -- Si habia 1+ antes y max_checkins > 1 -> INCREMENTAR (permite multiples)
  -- Si habia 1+ antes y max_checkins = 1 -> NO deberia llegar aqui (validacion previa)
  
  IF v_existing_checkins = 0 THEN
    -- Primer check-in del dia: SIEMPRE incrementar (excepto si racha completada/expirada/rota)
    v_should_increment := true;
  ELSIF v_max_checkins_daily > 1 THEN
    -- Multiples check-ins permitidos por dia: incrementar
    v_should_increment := true;
  ELSE
    -- Solo 1 check-in por dia y ya habia uno: no incrementar
    -- (Este caso no deberia ocurrir por la validacion anterior)
    v_should_increment := false;
  END IF;

  -- ============================================
  -- 9. ACTUALIZAR/CREAR RACHA
  -- ============================================
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    -- Recalcular expires_at para nueva temporada si expiro
    expires_at = CASE
  ...
  -- ============================================
  -- 10. OBTENER CURRENT_COUNT ACTUALIZADO
  -- ============================================
  ...
  -- ============================================
  -- 11. GENERAR CUPONES AUTOMATICOS POR THRESHOLD
  -- ============================================
  ...
  -- ============================================
  -- 12. VERIFICAR COMPLETACION DE RACHA MAXIMA
  -- ============================================
...
'Funcion de check-in CORREGIDA: usa conteo real de check_ins para decidir incremento de racha';
